...
var GPIO = module.exports = {
    ...
    setup_output: function (pin, val) {
        GPIO.write_pin(pin, val)
    },
    blink: function (pin, timeOn, timeOff) {
        if (timeOff == 0 && timeOff == 0) {
            if (blinkingTimers[pin] != undefined) {
                clearInterval(blinkingTimers[pin])
            }
            else {
                return;
            }
        }
        blinkingTimers[pin] = setInterval(() => {
            GPIO.write_pin(pin, 1);
            setTimeout(() => {
                GPIO.write_pin(pin, 0);
            }, timeOff)
        }, timeOn + timeOff)
    },
    setup_output: function (pin, pull) {
        GPIO.write_pin(pin, pull)
    },
    read: function (pin) {
        GPIO.read_pin()
